...
>
> qmux is bent hard toward how I personally work. It is not a general-purpose, supported
> product, and I make no promise to keep it stable or to accept issues/PRs.
>
> **The defining behavior:** qmux does **not** manage git for you. No forced worktrees, no branch
...
> (`QMUX_USE_WORKTREE=1`) and off by default.
...
qmux isn't published to npm — run it from source (see [CONTRIBUTING.md](./CONTRIBUTING.md)):

```bash
git clone https://github.com/daromaj/dmux qmux
cd qmux
pnpm install
pnpm run build
npm link   # exposes `qmux` on your PATH
```
...
qmux is a tmux + Ink TUI that opens a pane per task and launches an AI agent in it. It's a **pane manager**, not a git wrapper: branches, commits, and merges are the agent's job, not qmux's.
...
- **Configurable AI provider** &mdash; point branch-name/commit AI helpers at any provider (`QMUX_AI_PROVIDER`, `QMUX_AI_MODEL`, `QMUX_AI_BASE_URL`, `QMUX_AI_API_KEY`); DeepSeek preset included
...
- **Built-in file browser** &mdash; inspect a pane's directory, search files, and preview code or diffs without leaving qmux
...
Worktrees are off by default. Set `QMUX_USE_WORKTREE=1` to enable the mode where each new pane gets its own git worktree and branch. Without it, panes share the project directory and qmux does nothing to your git state.
...
This is a personal tool, so I'm not really soliciting contributions. If you want to hack on it locally, **[CONTRIBUTING.md](./CONTRIBUTING.md)** documents the "qmux-on-qmux" development loop.

## Credits

Originally based on the qmux project by the FormKit team.
