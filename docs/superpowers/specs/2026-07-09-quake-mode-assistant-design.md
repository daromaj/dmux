...
that talks to qmux's already-configured LLM and can **operate the workspace** as a basic
...
"set up panes for X") and the assistant drives qmux by sending keystrokes to panes,
...
teaches it how to operate qmux, and a ReAct-style execute→observe→continue loop. No
...
- **Action mechanism:** free-form. The model emits shell/tmux commands as text; qmux runs
...
- **Safety gate:** **none** (full auto). Every command the model emits runs without
  ...
  `.qmux/quake-history.jsonl` so there is a record of what ran. This risk posture is a
  deliberate choice.
- **Overlay:** in-process Ink drop-down (not a tmux popup).
- **History:** in-memory for the qmux process lifetime (survives overlay close/reopen);
  append-only transcript file for forensics. No rotation in v1.

## Why these over the alternatives

- **In-process Ink overlay over tmux popup:** qmux popups are separate Node processes,
  ...
  component rendered conditionally from `QmuxApp.tsx` can.
- **Free-form over native function-calling:** portable to any OpenAI-compatible endpoint
  including `custom`/DeepSeek; qmux's existing LLM path (`PaneAnalyzer`) already uses raw
...
- ` ```qmux ` — control verbs routed **into the running Ink process** so settings changes
  actually stick. Verbs (v1): `grid <auto|1|2|3|4>`, `control <bottom|left>`,
  `color <paneSlugOrId> <color>`, `layout refresh`.

### Why the `qmux:` lane is required (not optional)

Nothing watches the settings files. Live layout/grid/color/control-position changes only
take effect when in-process code calls `refreshQmuxSettings()` + `queueLayoutRefresh()`
(see `QmuxApp.tsx`, `useInputHandling.ts`). `ConfigWatcher` watches only the *panes* file
(`qmux.config.json`), not settings. And qmux's layout enforcer re-tiles on refresh/SIGWINCH,
...
| `src/utils/quakeCommands.ts` | Pure parser: assistant text → ordered blocks (`prose` / `shell` / `qmux-control`). | — |
| `src/utils/quakeControlVerbs.ts` | Maps `qmux:` verbs → in-process effects via an injected `SettingsManager` + refresh callbacks. | settings, callbacks |
| `src/utils/quakeSystemPrompt.ts` | Builds the system prompt (the qmux operating manual + live workspace context). | pane state, settings |
...
| wiring | `QmuxApp.tsx` renders the overlay + owns a service instance; `useInputHandling.ts` adds the toggle binding and swallows other input while open. | — |

## System prompt (the operating manual)

The system prompt is the heart of the feature. It contains:

1. **What qmux is** — a project-scoped tmux session managing parallel AI-agent work; one
...
2. **How to operate panes** — how to send a prompt/keystrokes to a pane
   ...
   to settle.
3. **How to control look-and-feel** — the `qmux:` control verbs and when to use them
   instead of raw tmux.
4. **The command protocol** — emit ` ```run ` blocks for shell/tmux, ` ```qmux ` blocks for
...
- **No API key configured** → overlay shows a setup hint (`QMUX_AI_API_KEY` /
...
- **Model quality** → tool-following tracks the configured model (default
  `google/gemini-2.5-flash`); point `QMUX_AI_MODEL` at a stronger model for best results.
  Noted, not solved.
...
- Persisting conversation across qmux restarts (in-memory only; transcript is forensic).
