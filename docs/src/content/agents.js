...
export function render() {
  return `
    <h1>Agents</h1>
    <p class="lead">qmux supports 12 AI coding agents. Each agent is automatically detected if its CLI is installed and available in your PATH.</p>

    <h2>Agent Detection</h2>
    <p>qmux automatically detects installed agents by searching:</p>
    <ol>
    ...
    <p>When creating panes from the TUI, qmux opens the agent chooser so you can choose one or more runs per agent. If <code>defaultAgent</code> is set in <a href="#/configuration">configuration</a>, that agent is preselected at <code>1x</code>; otherwise the first available agent is preselected.</p>
    ...
    <p>The <code>permissionMode</code> setting controls what flags qmux passes to each agent:</p>
    <table>
      <thead>
    ...
    <p>When <code>enableAutopilotByDefault</code> is enabled in <a href="#/configuration">settings</a>, qmux will automatically accept agent option dialogs when no risk is detected. This reduces manual intervention while agents work.</p>
    ...
    <p>qmux currently enables native goal launch behavior for Claude Code and Codex. Codex launches with the experimental <code>--enable goals</code> flag when goal mode is selected. Other agents receive the normal initial prompt unless they add compatible goal-mode support later.</p>

    <h2>Grok Build Notes</h2>
    <p>Grok Build is detected as <code>grok</code>. qmux starts the interactive TUI and pastes the initial prompt into it so the pane remains usable after the first response. Reopened Grok panes use <code>grok --continue</code> from the worktree directory.</p>
    <p>qmux also installs lightweight Grok project hooks in <code>.grok/hooks/</code> for stop and notification events. Grok requires project hook trust before project hooks execute; open Grok's hooks UI with <code>/hooks</code> or run <code>/hooks-trust</code> inside the Grok session if needed.</p>
    ...
    <p>qmux monitors each agent pane to determine its current state. This is used to show status indicators in the sidebar.</p>
    ...
    <ol>
      ...
      <li><strong>LLM analysis</strong> — when activity stops, qmux uses lightweight OpenRouter models, with a free JSON-capable fallback, to analyze the terminal content and determine if the agent is waiting for input, showing a dialog, or idle</li>
      <li><strong>User typing detection</strong> — if the user is typing, qmux avoids false positives</li>
    </ol>
