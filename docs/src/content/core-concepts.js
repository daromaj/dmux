...
export function render() {
  return `
    <h1>Core Concepts</h1>
    <p class="lead">qmux is built around three core concepts: projects, panes, and worktrees. Understanding how they interact will help you get the most out of parallel development.</p>

    <h2>Projects</h2>
    <p>A <strong>project</strong> in qmux corresponds to a git repository. When you run <code>qmux</code> inside a repo, it creates a stable, project-scoped tmux session named like <code>qmux-your-project-a1b2c3d4</code>.</p>
    <ul>
      <li>Each project gets its own isolated tmux session</li>
      <li>Session state is stored in <code>.qmux/qmux.config.json</code></li>
    ...
    <ul>
      <li>A unique ID (e.g. <code>qmux-1</code>, <code>qmux-2</code>)</li>
    ...
    <ol>
      ...
      <li><strong>Monitor</strong> — qmux tracks agent status (working, analyzing, waiting, idle)</li>
    ...
    <p>qmux monitors each pane to detect the agent's state:</p>
    <table>
      <thead>
      ...
      <tbody>
        ...
        <tr><td><code>analyzing</code></td><td>qmux is analyzing recent terminal output</td></tr>
    ...
    <p>When a pane settles into a state that needs you, qmux marks it with an attention indicator in the sidebar and pane border. On macOS, qmux can also send a native notification for panes that are not currently focused.</p>
    <ul>
      ...
      <li>On non-macOS platforms, qmux still works normally without the native helper</li>
    </ul>

    <h3>Pane Visibility</h3>
    <p>qmux lets you temporarily remove panes from the active window without stopping them.</p>
    <ul>
    ...
    <ul>
      ...
      <li>Preview either file contents or git diff output without leaving qmux</li>
...
└── .qmux/worktrees/
    ...
    <p>When you merge a pane (press <kbd>m</kbd> to open the menu, then select merge), qmux performs a two-phase merge:</p>
    <ol>
    ...
    <div class="callout callout-info">
      ...
      If there are merge conflicts in step 2, qmux will abort the merge and let you know which files conflict. You can resolve them manually in the worktree and retry.
    </div>

    <h2>The Sidebar</h2>
    <p>The qmux TUI shows a sidebar with all active panes. Each pane displays:</p>
    <ul>
    ...
    <p>You can optionally use a sidebar layout mode (press <kbd>L</kbd>) where the qmux sidebar occupies a fixed 40-column pane on the left, with agent panes arranged in a grid to the right.</p>
